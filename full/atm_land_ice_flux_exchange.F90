...
module atm_land_ice_flux_exchange_mod
  ...
  use atmos_tracer_driver_mod, only &
       atmos_tracer_has_surf_setl_flux, &
         !function returns True if tracer sedimentation flux > 0 at bottom of the atmosphere
  ...
  use FMS ! mainly for calls to diag_manager, data_override, and exchange
  ...
  character(len=128) :: version = '$Id$' !< coupler version number that is set automatically during compile time
  character(len=128) :: tag = '$Name$' !< coupler tag that is set automatically during compile time

  !> FmsXgridXmap_type that holds the exchange grid between different componentes
  ...
  integer :: &
       id_b_star, & !< diag_manager register field id for 'bouyancy scale'
       id_del_h, & !< diag_manager register field id for 'ref height interp factor for for heat'
       id_del_m, & !< diag_manager register field id for 'ref height interp factor for momentum'
       id_del_q, & !< diag_manager register field id for 'ref height interp factor for moisture'
       id_drag_heat, & !< diag_manager register field id for 'drag coefficient for heat'
       id_drag_moist, & !< diag_manager register field id for 'drag coefficient for moisture'
       id_drag_mom, & !< diag_manager register field id for 'drag coefficient for momentum'
       id_gust, & !< diag_manager register field id for 'gust scale'
       id_hussLut_land, & !<diag_manager register field id for 'near-surface specific humidity on land use tile'
       id_ice_mask, & !< diag_manager register field id for 'fractional amount of land'
       id_land_mask, & ! diag_manager register field id for 'fractional amount of sea ice'
       id_p_atm, & !< diag_manager register field id for 'pressure at lowest atmospheric level'
       id_q_flux, & !< diag_manager register field id for 'evaporation rate'
       id_q_flux_land, & !< diag_manager register field id for 'evaporation rate over land'
       id_q_ref, & !< diag_manager register field id for 'specific humidity at z_ref_heat'
       id_q_ref_land, & !< diag_manager register field id for 'specific humidity at z_ref_heat over land'
       id_q_star, & !< diag_manager register field id for 'moisture scale'
       id_r_flux, & !< diag_manager register field id for 'net (down-up) longwave flux'
       id_rh_ref, & !< diag_manager register field id for 'relative humidity at z_ref_heat'
       id_rh_ref_cmip, & !< diag_manager register field id for 'relative humidity at z_ref_heat'
       id_rh_ref_land, & !< diag_manager register field id for 'relative humidity at z_ref_heat over land'
       id_rough_heat, & !< diag_manager register field id for 'surface roughness for heat'
       id_rough_moist, & !< diag_manager register field id for 'surface roughness for moisture'
       id_rough_mom, & !< diag_manager register field id for 'surface roughness for momentum'
       id_rough_scale, & !< diag_manager register field id for 'topographic scaling fractor for momentum drag'
       id_slp, & !< diag_manager register field id for 'sea level pressure'
       ...
       id_t_flux_land, & !< diag_manager register field id for 'sensible heat flux over land'
       ...
       id_t_surf, & !< diag_manager register field id for 'surface temperature'
       id_tasLut_land, & !< diag_manager register field id for 'near-surface air temperature at
         !! z_ref_heat above displacement height on land-use tile'
       id_thv_atm, & !< diag_manager register field id for 'surface air virtual potential temperature'
       id_thv_surf, & !< diag_manager register field id for 'surface virtual potential temperature'
       id_u_atm, & !< diag_manager register field id for 'u wind component at lowest atmospheric level'
       id_u_flux, & !< diag_manager register field id for 'zonal wind stress'
       id_u_ref, & !< diag_manager register field id for 'zonal wind component at z_ref_mom'
       id_u_ref_land, & !< diag_manager register field id for 'zonal wind component at z_ref_mom over land'
       id_u_star, & !< diag_manager register field id for 'friction velocity'
       id_v_atm, & !< diag_manager register field id for 'v wind component at lowest atmospheric level'
       id_v_flux, & !< diag_manager register field id for 'meridional wind stress'
       id_v_ref, & !< diag_manager register field id for 'meridional wind component at z_ref_mom'
       id_v_ref_land, & !<diag_manager register field id for 'meridional wind component at z_ref_mom over land'
       id_wind, & !< diag_manager register field id for 'wind speed for flux calculations'
       ...
       id_o2_flux_pcair_atm !< diag_manager register field id for 'concentration of o2 to be passed to to ocean'

  integer, allocatable :: &
       id_tr_atm(:), & !< array of diag_manager register field ids for 'tracers at lowest atmospheric level'
       id_tr_surf(:), & !< array of diag_manager register field ids for 'tracers at surface'
       id_tr_flux(:), & !< array of diag_manager register field ids for 'tracers fluxes'
       id_tr_mol_flux(:), & !< array of diag_manager register field ids for 'flux of co2 concentration in [mol/m2*s]'
       id_tr_ref(:), & !< array of diag_manager register field ids for 'tracers at z_ref_heat'
       id_tr_ref_land(:), & !< array of diag_manager register field ids for 'tracer flux at z_ref_heat over land'
       !f1p
       id_tr_mol_flux0(:), &
  ...
  integer :: &
       id_evspsbl, & !< diag_manager register field id for 'water evaporation flux' (cmip)
       id_height10m, & !< diag_manager register field id for 'near surface height' (cmip)
       id_height2m, & !< diag_manager register field id for 'near surface height' (cmip)
       id_hfls, & !< diag_manager register field id for 'surface upward latent heat flux' (cmip)
       id_hfss, & !< diag_manager register field id for 'surface upward sensible heat flux' (cmip)
       id_hurs, & !< diag_manager register field id for 'near-surface relative humidty' (cmip)
       id_huss, & !< diag_manager register field id for 'near-surface specific humidity' (cmip)
       id_psl, & !< diag_manager register field id for 'air pressure at sea level' (cmip)
       id_rhs, & !< diag_manager register field id for 'near-surface relative humidty' (cmip)
       id_sfcWind, & !< diag_manager register field id for 'near-surface wind speed' (cmip)
       id_sftlf, & !< diag_manager register field id for 'fraction of the grid cell occupied by land' (cmip)
       id_sic, & !< diag_manager register field id for 'sea ice area fraction' (cmip)
       id_tas, & !< diag_manager register field id for 'near-surface air temperature' (cmip)
       id_tauu, & !< diag_manager register field id for 'surface downward eastward wind stress' (cmip)
       id_tauv, & !< diag_manager register field id for 'surface downward northward wind stress' (cmip)
       id_tos, & !< diag_manager register field id for 'sea surface temperature' (cmip)
       id_ts, & !< diag_manager register field id for 'surface temperature' (cmip)
       id_tslsi, & !< diag_manager register field id for 'surface temperature on land or sea ice' (cmip)
       id_uas, & !< diag_manager register field id for 'eastward near-surface wind' (cmip)
       id_vas !< diag_manager register field id for 'northward near-surface wind' (cmip)

  ! globally averaged diagnostics
  integer :: &
       id_evspsbl_g, & !< diag_manager register field id for 'global integral of water evaporation flux'
       id_hfls_g, & !< diag_manager register field id for 'global integral of surface upward latent heat flux'
       id_hfss_g, & !< diag_manager register field id for 'global integral of surface upward sensible heat flux'
       id_rls_g, & !< diag_manager register field id for 'global integral of near-surface relative humidty'
       id_tas_g, & !< diag_manager register field id for 'global integral of near-surface air temperature'
       id_tasl_g, & !< diag_manager register field id for 'global integral of near-surface air temperature on land'
       id_ts_g !< diag_manager register field id for 'global integral of surface temperature'
  ...
  logical :: do_area_weighted_flux = .FALSE. !< not used variable
  logical :: do_forecast = .false. !< do_forecast 
  ...
  real, allocatable, dimension(:) :: &
       ...
       ex_albedo_fix, & !< albedo_fix on the exchange grid (TO CLARIFY)
       ex_albedo_nir_dif_fix, & !< albedo_nir_dif_fix on the exchange grid (TO CLARIFY)
       ex_albedo_nir_dir_fix, & !< albedo_nir_dir_fix on the exchange grid (TO CLARIFY)
       ex_albedo_vis_dif_fix, & !< albedo_vis_dif_fix on the exchange grid (TO CLARIFY)
       ex_albedo_vis_dir_fix, & !< albedo_vis_dir_fix on the exchange grid (TO CLARIFY)
       ex_b_star, & !< boyuancy scale on the exchange grid
       ex_cd_m, & !< drag coefficient for momentum on the exchange grid
       ex_cd_t, & !<  drag coefficient for heat on the exchange grid
       ex_con_atm, & !< deposition velocity at lowest atmospheric level on the exchange grid
       ex_dedt_surf, & !< d(water.vap.flux)/d(T canopy) on the exchange grid
       ...
       ex_e_q_n, & !< dt/mass * dedet_surf * gamma on the exchange grid
       ex_flux_lw, &  !< longwave radiation flux on the exchange grid
       ex_flux_t, & !< sens heat flux on the exchange grid
       ex_flux_u, & !< u stress on atmosphere on the exchange grid
       ex_flux_v, & !< v stress on atmosphere on the exchange grid
       ex_old_albedo, & !< old value of albedo for downward flux calculations on the exchange grid
       ex_p_surf, &  !< surface pressure on exchange grid on the exchange grid
       ex_seawater, & !< mask array of seaice fractions on the exchange grid
       ...
       ex_t_surf_miz, & !< array on the exchange grid (TO CLARIFY)
  ...
  real, allocatable, dimension(:,:) :: &
       ex_dfdtr_atm, & !< d(tracer flux)/d(atm tracer) on the exchange grid
       ex_dfdtr_surf, & !< d(tracer flux)/d(surf tracer) on the exchange grid
       ex_e_tr_n, & !< coefficient in implicit scheme on the exchange grid
       ex_f_tr_delt_n, & !< coefficient in implicit scheme on the exchange grid
       ex_flux_tr, & !< tracer fluxes on the exchange grid
       ex_tr_con_ref, & !< deposition velocity at reference height on the exchange grid
       ex_tr_con_atm, & !< deposition velocity at atmospheric height on the exchange grid
       ex_tr_surf !< near-surface tracer fields  on the exchange grid

  logical, allocatable, dimension(:) :: &
       ex_avail, & !< true where data on exchange grid are available
       ex_land !< true if exchange grid cell is over land
  real, allocatable, dimension(:) :: &
       ex_e_t_n, & !< temporary array on the exchange grid (TO CLARIFY)
       ex_f_t_delt_n !< temporary array on the exchange grid (TO CLARIFY)
  ...
  type(fmscoupler1dbc_type), pointer :: ex_gas_fields_atm=>NULL() !< atm gas place holder
  ...
  integer, parameter :: &
       regrid=1, & !< boundary_type%xtype where grids are physically different, pass via exchange grid
       redist=2, & !< boundary_type%xtype where same physical grid, different decomposition, must move data around
       redirect=3 !< boundary_type%xtype same physical grid, same domain decomposition, can directly copy data

  integer :: &
       cplClock, & !< FMS clock id for profiling general processes
       sfcClock, & !< FMS clock id for profiling sfc_boundary_layer
       fluxAtmDnClock, & !< FMS clock id for profiling flux down from atmosphere
       regenClock, & !< FMS clock for profiling exchange grid generation
       fluxAtmUpClock !< FMS clock for profiling flux up to atmosphere
  ...
  integer :: &
       ni_atm, & !< number of x gridpoints in the atm compute domain
       nj_atm !< number of y gridpoints in the atm compute domain

  integer :: &
       nxc_ice=0, & !< number of x gridpoints in ice compute domain
       nyc_ice=0, & !< number of y gridpoints in ice compute domain
       nk_ice=0 !< number of vertical levels in ice

  integer :: &
       nxc_lnd=0, & !< number of x gridpoints in land compute domain
       nyc_lnd=0 !< number of y gridpoints in land compute domain

contains

  !#######################################################################
  !! Subroutine atm_land_ice_flux_exchange_init initializes the atm_land_ice_flux_exchange_mod by 
  ...
  subroutine atm_land_ice_flux_exchange_init(Time, Atm, Land, Ice, atmos_ice_boundary, land_ice_atmos_boundary, &
    ...
    logical, intent(in) :: scale_precip_2d_in
      !< if true, rescale Atm%lprec
    logical, intent(in) :: do_area_weighted_flux_in
      !< if true, divide flux by area
    logical, intent(in) :: do_forecast_in
      !< if true, and AM3_physics is define, put atm%surf_diff%sst_miz on the exchange grid
    ...
    integer, intent(in) :: nblocks_in
      !< divide the surface exchange grid to nblocks for OpenMP parallelization
    ...
    !> INITIALIZE DT_ATM, DT_CPL, Z_REF_HEAT, Z_REF_MOM, DO_AREA_WEIGHTED_FLUX,
    !! DO_FORECAST, PARTITION_FPREC_FROM_LPREC, SCALE_PRECIP_2D, NBLOCKS, CPLCLOCKS
    !! EX_GAS_FIELDS_ATM, EX_GAS_FIELDS_ICE, AND EX_GAS_FLUXES
    !{
    Dt_atm = Dt_atm_in
    Dt_cpl = Dt_cpl_in
    ...
    !> CONSTRUCT THE TRACER TABLE (TR_TABLE): FOR EACH TRACER, RECORD THE TRACER_INDEX IN THE ATM MODEL, ICE MODEL,
    !! AND LAND MODEL.  SKIP ALL ATMOS TRACERS THAT DOES NOT HAVE CORRESPONDING SURFACE TRACERS
    !{
    ...
    !> GET THE TOTAL NUMBER OF GENERIC EXCHANGE BETWEEN ATMOSPHERE AND LAND
    !! TODO: DEFINE GENERIC EXCHANGE FIELD
    !{
    ...
    !> CONSTRUCT THE TRACER TABLE (TR_TABLE):  FOR EACH OCEAN-ATM GAS FLUX TRACER, STORE THE TRACER INDEX
    !! NOTE, THE NAMES OF THE TRACERS IN ATMOSPHERE AND OCEAN MAY DIFFER
    !{
    ...
    if (isphum==NO_TRACER) then
       call fms_error_mesg(module_name, 'tracer "sphum" must be present in the atmosphere', FATAL)
    endif

    if (ico2==NO_TRACER) then
       call fms_error_mesg(module_name, 'tracer "co2" not present in the atmosphere', NOTE)
    endif
    !}
...
#ifndef _USE_LEGACY_LAND_
    call fms_xgrid_setup_xmap(xmap_sfc, ['ATM', 'OCN', 'LND'], [Atm%Domain, Ice%Domain, Land%Domain], &
    ...
    !> INITIALIZE SURFACE_FLUX MODULE IN FMSCOUPLER/SHARED
    call surface_flux_init()

    !> INITIALLIZE FMS DIAG_INTEGRAL FIELDS FOR GLOBAL INTEGRAL QUANTITIES
    !{
  ...
  !}
    ...
    if (.not.fms_coupler_type_initialized(Ice%ocean_fields)) &
         call fms_coupler_type_spawn(ex_gas_fields_ice, Ice%ocean_fields, &
         [is,is,ie,ie], [js,js,je,je], [1, kd], suffix = '_ice')
    call fms_coupler_type_set_diags(Ice%ocean_fields, 'ice_flux', Ice%axes, Time)
    !} END ALLOCATE ATMOS_ICE_BOUNDARY
    ...
    allocate(land_ice_atmos_boundary%rough_heat(is:ie,js:je), source=0.01) ! Kun
  ...
  !! the exchange grid.  Fluxes are computed on the exchange grid.  Then the data is mapped from the
  !! exchange grid to the receiving model component grid. However, computed fields and fluxes can be overwritten
  ...
  subroutine sfc_boundary_layer (dt, Time, Atm, Land, Ice, Land_Ice_Atmos_Boundary)
...
#endif
    real, dimension(size(Ice%t_surf,1),size(Ice%t_surf,2),size(Ice%t_surf,3)) :: sea !< temporary array
    real, dimension(size(Ice%albedo,1),size(Ice%albedo,2),size(Ice%albedo,3)) ::  tmp_open_sea
      ! temporary array used in generating ice/ocean mask

    real :: &
         zrefm, & ! reference height for computing surface fluxes with Monin-Obukhov similarity theory
         zrefh ! reference height for computing surface fluxes with Monin-Obukhov similarity theory
    ...
    !> ALLOCATE ARRAY FOR EXCHANGE FIELDS.  THE ARRAYS WILL BE DEALLOCATED IN FLUX_UP_TO_ATMOS
    !{
    allocate ( &
    ...
    !> SET INITIAL VALUES FOR A SUBSET OF EXCHANGE FIELDS (OPENMP LOOP)
    !{
    ...
    do l=1, my_nblocks
       ...
       enddo
    enddo
    !} END COMPUTE EXPLICIT FLUXES BETWEEN ATM AND OCEAN
...
! kgao: for shield+mom6 coupling; used by shield pbl schemes (am5 with tke-edmf should do the same)
...
#endif
    !} END MAP FROM EXCHANGE TO ATM GRID
    ...
    !! DATA_OVERRIDE WILL ONLY OVERWRITE XARRAY IF THE FIELD IS SPECIFIED IN THE DATA_TABLE
    !{
    ...
    !$OMP ex_rough_heat,ex_t_ca,ex_t_atm,ex_p_surf,ex_qs_ref,ex_del_h, ex_ref,ex_qs_ref_cmip,ex_ref2) &
    !$OMP private(is,ie)
    do l = 1, my_nblocks
       is=block_start(l)
       ie=block_end(l)
  ...
  subroutine flux_down_from_atmos(Time, Atm, Land, Ice, Atmos_boundary, Land_boundary, Ice_boundary)
    ...
    real, dimension(n_xgrid_sfc,n_gex_atm2lnd) ::  ex_gex_atm2lnd ! generic exchange fields between atm and land
    ...
    !! NOTE, THE PARTITIONING ONLY OCCURS ON MPI RANKS FOR ATMOSPHERE
    ...
    !} END MAP ATMOSPHERIC QUANTITIES ON EXCHANGE GRID
    ...
    do l = 1, my_nblocks
       ...
       enddo
    enddo
    !} end update u and v stress on the exchange grid

    !> ON THE EXCHANGE GRID, FIX SHORTWAVE RADIATION FLUX OF VISIBLE LIGHT TO ACCOUNT FOR ALBEDO VARIATION
    !{
    ...
    !} ON THE EXCHANGE GRID, END FIX SHORT WAVELENGTH RADIATION FLUX


    !> ADJUST FLUXES FOR IMPLICIT DEPENDENCE
    !{
    do tr = 1,n_exch_tr
       n = tr_table(tr)%atm
    ...
    !$OMP ex_avail, ex_gamma, ex_dtmass, ex_dflux_t, ex_dhdt_atm, cp_inv, ex_e_t_n, &
    !$OMP ex_dhdt_surf,ex_f_t_delt_n,ex_delta_t, ex_flux_t, ex_dflux_tr, isphum, &
    !$OMP ex_dfdtr_atm, ex_e_q_n, ex_dedt_surf, n_exch_tr, ex_e_tr_n, ex_dfdtr_surf, &
    !$OMP ex_f_tr_delt_n, ex_delta_tr, ex_flux_tr) private(is,ie)
    do l = 1, my_nblocks
       is=block_start(l)
       ie=block_end(l)
       do i = is, ie
       ...
       enddo
    enddo
    !} END ADJUST FLUXES FOR IMPLICIT DEPENDENCE
    ...
    enddo
    !} END MAP ICE FIELDS FROM THE EXCHANGE GRID TO THE ICE GRID
  ...
  !> Subroutine generate_sfc_xgrid updates the fractional area of the exchange grid between land and ice, where
  ...
  subroutine flux_up_to_atmos ( Time, Land, Ice, Land_Ice_Atmos_Boundary, Land_boundary, Ice_boundary )
    ...
    real, dimension(n_xgrid_sfc) ::  &
         ex_t_surf_new, & ! new surface temperature on the exchange grid
         ex_dt_t_surf,  & !
         ex_delta_t_n,  &
         ex_t_ca_new,   &
         ex_dt_t_ca,    &
    ...
    ! jgj: added for co2_surf diagnostic, where co2_surf_dvmr is the updated CO2 tracer values at the surface (dry vmr)
...
#else
    real, dimension(size(Land_boundary%lprec,1), size(Land_boundary%lprec,2), size(Land_boundary%lprec,3)) :: &
         data_lnd, diag_land
#endif
    ...
    !> PUT ICE%T_SURF, LAND%T_CA AND LAND%T_SURF ONTO THE EXCHANGE GRID
    !{
    ...
    do l = 1, my_nblocks
       ...
       !} END COMPUTE CHANGE IN SURFACE TEMPERATURE
    ...
    enddo
    !} END UPDATE FLUXES AND ATMOSPHERIC INCREMENTS FOR IMPLICIT DEPENDENCE ON SURFACE TEMPERATURE
    ...
    do l = 1, my_nblocks
       ...
       enddo
    enddo
    !} END UPDATE TRACER TENDENCIES IN THE ATMOSPHERE
...
#endif
    !} END MAP DT_T, SHFLX, and LHFLX FIELDS IN LAND_ICE_ATMOS_BOUNDARY FROM THE EXCHANGE GRID TO THE ATMOSPERE GRID
...
#else
    call fms_xgrid_get_from_xgrid (diag_atm, 'ATM', ex_t_surf_new, xmap_sfc)
    if (id_t_surf > 0) used = fms_diag_send_data(id_t_surf, diag_atm, Time)
    if (id_ts > 0) used = fms_diag_send_data(id_ts, diag_atm, Time)
    ...
    do tr=1, n_exch_tr
       if(id_tr_flux(tr) > 0 .or. id_tr_mol_flux(tr) > 0) then
          ...
          if(id_tr_flux(tr) > 0 ) used = fms_diag_send_data ( id_tr_flux(tr), diag_atm, Time)
    ...
    do tr=1, n_exch_tr
       if(id_tr_flux_land(tr) > 0 .or. id_tr_mol_flux_land(tr) > 0) then
          ...
          if(id_tr_flux_land(tr) > 0) call send_tile_data(id_tr_flux_land(tr), diag_land)
...
#endif
    !} END SEND DATA TO DIAG_MANAGER_BUFFER
  ...
  !> Subroutine flux_ex_arrays_dealloc deallocates arrays holding fields on the exchange grid
  ...
  !! and is called only if the do_flux namelist variable is set to .True.
  subroutine flux_atmos_to_ocean(Time, Atm, Ice_boundary, Ice)
    ...
    !> MAP AIR_SEA_DEPOSITION FLUX FROM THE EXCHANGE GRID TO THE ICE GRID
    !! BUT IF THE FLUX FIELD EXISTS IN DATA_TABLE, OVERWRITE DATA WITH
    !! DATA_OVERRIDE.  IF THE FIELD IS REGSITERED IN DIAG_MANAGER, SEND DATA
    !! TO THE DIAG_MANAGER BUFFER
    !{
    do n = 1, Ice_boundary%fluxes%num_bcs
  ...
  !> UPDATE ICE FIELDS THAT ARE LABELED AS AIR_SEA_DEOOSITION FLUXES
  call update_ice_atm_deposition_flux(Ice_boundary, Ice)

  end subroutine flux_atmos_to_ocean

  !> \brief Converts 3D logical mask arrays to real arrays with values of 1.0 for true and
  !! 0.0 for false before mapping the mask to the exchange grid
  !! Internally used subroutine to convert Land%mask when using _USE_LEGACY_LAND_
  subroutine put_logical_to_real_sg (mask, id, ex_mask, xmap)

    implicit none
    logical, intent(in) :: mask(:,:,:) !< land/ice mask to map to exchange grid
    character(len=3), intent(in) :: id
    real, intent(inout) :: ex_mask(:) !< converted mask on exchange grid
  ...
  !> \brief Converts 2D logical mask arrays to real arrays with values of 1.0 for true and
  !! 0.0 for false before mapping the mask to the exchange grid
  !! Internally used subroutine to convert Land%mask when not using _USE_LEGACY_LAND_
  subroutine put_logical_to_real_ug (mask, id, ex_mask, xmap)

    implicit none
    logical, intent(in) :: mask(:,:) !< mask on component grid
    character(len=3), intent(in) :: id
    real, intent(inout) :: ex_mask(:) !< converted mask on exchange grid
    type(FmsXgridXmap_type), intent(inout) :: xmap
  ...
  !! Note, All diagnostic fields must be registered in diag_manager and all diagnostics fields
  ...
  subroutine diag_field_init(Time, atmos_axes, land_axes, land_pe)
    ...
    integer, intent(in) :: atmos_axes(2) !< array size for atmospheric diagnostic fields
    integer, intent(in) :: land_axes(:) !< array size for land diagnostic fields
    logical, intent(in) :: land_pe !< land pe number
...
110 format (f4.1,' m')
    ...
    id_ice_mask = fms_diag_register_diag_field(&
         mod_name, 'ice_mask', atmos_axes, Time, 'fractional amount of sea ice', 'none', range=frange, &
         interp_method = "conserve_order1")

    id_wind = fms_diag_register_diag_field(&
         mod_name, 'wind', atmos_axes, Time, 'wind speed for flux calculations', 'm/s', range=[0.,vrange(2)])

    id_drag_moist = fms_diag_register_diag_field(&
         mod_name, 'drag_moist', atmos_axes, Time, 'drag coeff for moisture', 'none')

    id_drag_heat = fms_diag_register_diag_field(&
         mod_name, 'drag_heat', atmos_axes, Time, 'drag coeff for heat', 'none')

    id_drag_mom = fms_diag_register_diag_field(&
         mod_name, 'drag_mom',  atmos_axes, Time, 'drag coeff for momentum', 'none')

    id_rough_moist = fms_diag_register_diag_field(&
         mod_name, 'rough_moist', atmos_axes, Time, 'surface roughness for moisture', 'm')

    id_rough_heat = fms_diag_register_diag_field(&
         mod_name, 'rough_heat', atmos_axes, Time, 'surface roughness for heat', 'm')

    id_rough_mom = fms_diag_register_diag_field(&
         mod_name, 'rough_mom', atmos_axes, Time, 'surface roughness for momentum', 'm')

    id_u_star  = fms_diag_register_diag_field(&
         mod_name, 'u_star', atmos_axes, Time, 'friction velocity', 'm/s')

    id_b_star = fms_diag_register_diag_field(&
         mod_name, 'b_star', atmos_axes, Time, 'buoyancy scale', 'm/s2')

    id_q_star = fms_diag_register_diag_field(&
         mod_name, 'q_star', atmos_axes, Time, 'moisture scale', 'kg water/kg air')

    id_thv_atm = fms_diag_register_diag_field(&
         mod_name, 'thv_atm', atmos_axes, Time, 'surface air virtual potential temperature', 'K')

    id_thv_surf = fms_diag_register_diag_field(&
         mod_name, 'thv_surf', atmos_axes, Time, 'surface virtual potential temperature', 'K')

    id_u_flux = fms_diag_register_diag_field(&
         mod_name, 'tau_x', atmos_axes, Time, 'zonal wind stress', 'pa')

    id_v_flux = fms_diag_register_diag_field(&
         mod_name, 'tau_y', atmos_axes, Time, 'meridional wind stress', 'pa')

    id_t_ocean = fms_diag_register_diag_field(&
         mod_name, 't_ocean', atmos_axes, Time, 'surface temperature from ocean output', 'deg_k', range=trange)

    id_t_surf = fms_diag_register_diag_field(&
         mod_name, 't_surf', atmos_axes, Time, 'surface temperature','deg_k', range=trange)

    ! + slm, Mar 25, 2002 -- add diagnositcs for t_ca, q_ca, and q_atm
    id_t_ca  = fms_diag_register_diag_field(&
         mod_name, 't_ca', atmos_axes, Time, 'canopy air temperature', 'deg_k', range=trange)

    ! - slm, Mar 25, 2002
    id_z_atm  = fms_diag_register_diag_field(&
         mod_name, 'z_atm', atmos_axes, Time, 'height of btm level', 'm')

    id_p_atm = fms_diag_register_diag_field(&
         mod_name, 'p_atm', atmos_axes, Time, 'pressure at btm level','pa')

    ! - bw, Mar 25, 2002 -- added diagnostic slp
    id_slp = fms_diag_register_diag_field(&
         mod_name, 'slp', atmos_axes, Time, 'sea level pressure', 'pa')

    id_gust = fms_diag_register_diag_field(&
         mod_name, 'gust', atmos_axes, Time, 'gust scale', 'm/s')

    id_t_flux = fms_diag_register_diag_field(&
         mod_name, 'shflx', atmos_axes, Time, 'sensible heat flux', 'w/m2')

    id_r_flux = fms_diag_register_diag_field(&
          mod_name, 'lwflx', atmos_axes, Time, 'net (down-up) longwave flux', 'w/m2')

    id_t_atm  = fms_diag_register_diag_field(&
         mod_name, 't_atm', atmos_axes, Time, 'temperature at btm level', 'deg_k', range=trange)

    id_u_atm = fms_diag_register_diag_field(&
         mod_name, 'u_atm', atmos_axes, Time, 'u wind component at btm level', 'm/s', range=vrange)

    id_v_atm = fms_diag_register_diag_field(&
         mod_name, 'v_atm', atmos_axes, Time, 'v wind component at btm level', 'm/s', range=vrange)

    id_t_ref = fms_diag_register_diag_field(&
         mod_name, 't_ref', atmos_axes, Time, 'temperature at '//label_zh, 'deg_k', range=trange)

    id_rh_ref = fms_diag_register_diag_field(&
         mod_name, 'rh_ref', atmos_axes, Time, 'relative humidity at '//label_zh, 'percent')

    id_rh_ref_cmip = fms_diag_register_diag_field(&
         mod_name, 'rh_ref_cmip', atmos_axes, Time, 'relative humidity at '//label_zh, 'percent')

    id_u_ref = fms_diag_register_diag_field(&
         mod_name, 'u_ref', atmos_axes, Time, 'zonal wind component at '//label_zm, 'm/s', range=vrange)

    id_v_ref = fms_diag_register_diag_field(&
         mod_name, 'v_ref', atmos_axes, Time, 'meridional wind component at '//label_zm, 'm/s', range=vrange)

    id_wind_ref = fms_diag_register_diag_field(&
         mod_name, 'wind_ref', atmos_axes, Time, 'absolute value of wind at '//label_zm, 'm/s', range=vrange)

    id_del_h = fms_diag_register_diag_field(&
         mod_name, 'del_h', atmos_axes, Time, 'ref height interp factor for heat', 'none')

    id_del_m = fms_diag_register_diag_field(&
         mod_name, 'del_m', atmos_axes, Time, 'ref height interp factor for momentum', 'none')

    id_del_q = fms_diag_register_diag_field(&
         mod_name, 'del_q', atmos_axes, Time, 'ref height interp factor for moisture','none')

    if(land_pe) then
       ! set the default filter (for area and subsampling) for consequent calls to register_tiled_diag_field
#ifndef _USE_LEGACY_LAND_
       call set_default_diag_filter('land')
#endif
       id_t_ref_land = FMS_DIAG_REGISTER_FIELD_(&
            'flux_land', 't_ref', Land_axes, Time, 'temperature at '//trim(label_zh)//' over land', 'deg_k', &
            range=trange, missing_value=-100.0)

       id_q_ref_land = FMS_DIAG_REGISTER_FIELD_(&
            'flux_land', 'q_ref', Land_axes, Time, 'specific humidity at '//trim(label_zh)//' over land', 'kg/kg', &
            missing_value=-1.0)

       id_rh_ref_land= FMS_DIAG_REGISTER_FIELD_(&
            'flux_land', 'rh_ref', Land_axes, Time, 'relative humidity at '//trim(label_zh)//' over land', 'percent', &
            missing_value=-999.0)

       id_u_ref_land = FMS_DIAG_REGISTER_FIELD_(&
            'flux_land', 'u_ref',  Land_axes, Time, 'zonal wind component at '//trim(label_zm)//' over land',  'm/s', &
            range=vrange, missing_value=-999.0)

       id_v_ref_land = FMS_DIAG_REGISTER_FIELD_( &
            'flux_land', 'v_ref',  Land_axes, Time, 'meridional wind component at '//trim(label_zm)//' over land', &
            'm/s', range=vrange, missing_value = -999.0)

       id_q_flux_land = FMS_DIAG_REGISTER_FIELD_(&
             'flux_land', 'evap', Land_axes, Time, 'evaporation rate over land', 'kg/m2/s', missing_value=-1.0)

       id_t_flux_land = FMS_DIAG_REGISTER_FIELD_(&
            'flux_land', 'shflx', Land_axes, Time, 'sensible heat flux', 'W/m2', missing_value=-1.0)

       id_tasLut_land = FMS_DIAG_REGISTER_FIELD_(&
            'cmor_land', 'tasLut', Land_axes, Time, &
       ...
       id_hussLut_land = FMS_DIAG_REGISTER_FIELD_(&
            'cmor_land', 'hussLut', Land_axes, Time, 'Near-Surface Specific Humidity on Land Use Tile', '1.0', &
       ...
       do tr = 1, n_exch_tr
          ...
          if(tr .ne. isphum) then
             id_tr_ref_land(tr) = FMS_DIAG_REGISTER_FIELD_('flux_land', trim(name)//'_ref', &
                  Land_axes, Time, trim(longname)//' at '//trim(label_zh)//' over land', trim(units),missing_value=-1.0)
          else
             id_tr_ref_land(tr) = -1
          end if
#endif
    ...
    id_q_ref = fms_diag_register_diag_field(&
         mod_name, 'q_ref', atmos_axes, Time, 'specific humidity at '//trim(label_zh), 'kg/kg', missing_value=-1.0)

    id_rough_scale = fms_diag_register_diag_field(&
         mod_name, 'rough_scale', atmos_axes, Time, 'topographic scaling factor for momentum drag','1')
    ...
    do tr = 1, n_exch_tr
       ...
       if ( fms_mpp_lowercase(trim(name))=='co2') then
          ...
          ! saving incorrect results (mol fluxes for other tracers computed with CO2 molar mass
    ...
    id_height2m = fms_diag_register_static_field(&
         mod_name, 'height2m', [null_axis_id], 'Height', 'm', standard_name = 'height')
    ...
    id_height10m = fms_diag_register_static_field(&
         mod_name, 'height10m', [null_axis_id], 'Height', 'm', standard_name = 'height')
...
#ifdef use_AM3_physics
    id_tas = fms_diag_register_diag_field (&
         mod_name, 'tas', atmos_axes, Time, 'Near-Surface Air Temperature', 'K' , &
         standard_name = 'air_temperature', area=area_id, missing_value=CMOR_MISSING_VALUE, range=trange)

    if (id_tas > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_tas, 'coordinates', 'height2m')

    id_uas = fms_diag_register_diag_field(&
         mod_name, 'uas', atmos_axes, Time, 'Eastward Near-Surface Wind', 'm s-1', &
         standard_name = 'eastward_wind', area=area_id, missing_value=CMOR_MISSING_VALUE, range=vrange)

    if(id_uas > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_uas, 'coordinates', 'height10m')

    id_vas = fms_diag_register_diag_field(&
         mod_name, 'vas', atmos_axes, Time, 'Northward Near-Surface Wind', 'm s-1', &
         standard_name = 'northward_wind', area=area_id, missing_value=CMOR_MISSING_VALUE, range=vrange)

    if(id_vas > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_vas, 'coordinates', 'height10m')

    id_sfcWind = fms_diag_register_diag_field(mod_name, 'sfcWind', atmos_axes, Time, &
         'Near-Surface Wind Speed', 'm s-1', standard_name = 'wind_speed', area=area_id, &
         missing_value=CMOR_MISSING_VALUE, range=vrange)

    if(id_sfcWind > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_sfcWind, 'coordinates', 'height10m')

    id_huss = fms_diag_register_diag_field(mod_name, 'huss', atmos_axes, Time, &
         'Near-Surface Specific Humidity', '1.0', standard_name = 'specific_humidity', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    if(id_huss > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_huss, 'coordinates', 'height2m')

    id_hurs = fms_diag_register_diag_field(mod_name, 'hurs', atmos_axes, Time, &
         'Near-Surface Relative Humidity', '%', standard_name = 'relative_humidity', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    if(id_hurs > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_hurs, 'coordinates', 'height2m')

    id_rhs = fms_diag_register_diag_field(mod_name, 'rhs', atmos_axes, Time, &
         'Near-Surface Relative Humidity', '%', standard_name = 'relative_humidity', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    if(id_rhs > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_rhs, 'coordinates', 'height2m')

    id_ts = fms_diag_register_diag_field(mod_name, 'ts', atmos_axes, Time, &
         'Surface Temperature', 'K', standard_name = 'surface_temperature', area=area_id, &
         missing_value=CMOR_MISSING_VALUE, range=trange)

    id_psl = fms_diag_register_diag_field(mod_name, 'psl', atmos_axes, Time, &
         'Sea Level Pressure', 'Pa', standard_name = 'air_pressure_at_sea_level', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    id_tauu = fms_diag_register_diag_field(mod_name, 'tauu', atmos_axes, Time, &
         'Surface Downward Eastward Wind Stress', 'Pa', standard_name = 'surface_downward_eastward_stress', &
         area=area_id, missing_value=CMOR_MISSING_VALUE)

    id_tauv = fms_diag_register_diag_field(mod_name, 'tauv', atmos_axes, Time, &
         'Surface Downward Northward Wind Stress', 'Pa', standard_name = 'surface_downward_northward_stress', &
         area=area_id, missing_value=CMOR_MISSING_VALUE)

    id_hfss = fms_diag_register_diag_field(mod_name, 'hfss', atmos_axes, Time, &
         'Surface Upward Sensible Heat Flux', 'W m-2', standard_name = 'surface_upward_sensible_heat_flux', &
         area=area_id, missing_value=CMOR_MISSING_VALUE)

    id_hfls = fms_diag_register_diag_field(mod_name, 'hfls', atmos_axes, Time, &
    ...
    id_evspsbl = fms_diag_register_diag_field(mod_name, 'evspsbl', atmos_axes, Time, &
         'Evaporation', 'kg m-2 s-1', standard_name = 'water_evaporation_flux', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    id_sftlf = fms_diag_register_static_field(mod_name, 'sftlf', atmos_axes,  &
         'Fraction of the Grid Cell Occupied by Land', '1.0', standard_name = 'land_area_fraction', area=area_id, &
         interp_method = "conserve_order1")

    id_tslsi = fms_diag_register_diag_field(mod_name, 'tslsi', atmos_axes, Time,  &
         'Surface Temperature Where Land or Sea Ice', 'K', standard_name = 'surface_temperature', area=area_id, &
         mask_variant=.true., missing_value=CMOR_MISSING_VALUE)

    id_tos = fms_diag_register_diag_field(mod_name, 'tos', atmos_axes, Time,  &
         'Sea Surface Temperature', 'K', standard_name = 'sea_surface_temperature', area=area_id, &
         mask_variant=.true., missing_value=CMOR_MISSING_VALUE)

    id_sic = fms_diag_register_diag_field(mod_name, 'sic', atmos_axes, Time,  &
         'Sea Ice Area Fraction', '1.0', standard_name = 'sea_ice_area_fraction', area=area_id, &
         missing_value=CMOR_MISSING_VALUE)

    if ( id_sic > 0 ) &
         call fms_diag_field_add_attribute(id_sic, 'comment', 'averaged over the ocean portion of grid box')
#else
    id_tas = register_cmip_diag_field_2d(mod_name, 'tas', Time, &
         'Near-Surface Air Temperature', 'K' , standard_name='air_temperature')

    if(id_tas > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_tas, 'coordinates', 'height2m')

    id_uas = register_cmip_diag_field_2d(mod_name, 'uas', Time, &
         'Eastward Near-Surface Wind', 'm s-1', standard_name='eastward_wind')

    if(id_uas > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_uas, 'coordinates', 'height10m')

    id_vas = register_cmip_diag_field_2d(mod_name, 'vas', Time, &
         'Northward Near-Surface Wind', 'm s-1', standard_name='northward_wind')

    if(id_vas > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_vas, 'coordinates', 'height10m')

    id_sfcWind = register_cmip_diag_field_2d(mod_name, 'sfcWind', Time, &
         'Near-Surface Wind Speed', 'm s-1', standard_name='wind_speed')

    if(id_sfcWind > 0 .and. id_height10m > 0) call fms_diag_field_add_attribute(id_sfcWind, 'coordinates', 'height10m')

    id_huss = register_cmip_diag_field_2d(mod_name, 'huss', Time, &
         'Near-Surface Specific Humidity', '1.0', standard_name='specific_humidity')

    if(id_huss > 0 .and. id_height2m > 0) call fms_diag_field_add_attribute(id_huss, 'coordinates', 'height2m')

    id_hurs = register_cmip_diag_field_2d(mod_name, 'hurs', Time, &
         'Near-Surface Relative Humidity', '%', standard_name='relative_humidity')
    ...
    id_ts = register_cmip_diag_field_2d(mod_name, 'ts', Time, &
         'Surface Temperature', 'K', standard_name='surface_temperature')

    id_psl = register_cmip_diag_field_2d(mod_name, 'psl', Time, &
         'Sea Level Pressure', 'Pa', standard_name='air_pressure_at_sea_level')
    ...
    id_evspsbl = register_cmip_diag_field_2d(mod_name, 'evspsbl', Time, &
         'Evaporation', 'kg m-2 s-1', standard_name='water_evaporation_flux')

    id_sftlf = fms_diag_register_static_field(mod_name, 'sftlf', atmos_axes, &
         'Fraction of the Grid Cell Occupied by Land', '1.0', standard_name='land_area_fraction', &
         area=area_id, interp_method='conserve_order1')

    id_tslsi = register_cmip_diag_field_2d(mod_name, 'tslsi', Time, &
         'Surface Temperature Where Land or Sea Ice', 'K', standard_name='surface_temperature', mask_variant=.true.)
    ...
    id_tos = register_cmip_diag_field_2d(mod_name, 'tos', Time, &
         'Sea Surface Temperature', 'K', standard_name='sea_surface_temperature', mask_variant=.true.)

    id_sic = register_cmip_diag_field_2d(mod_name, 'sic', Time,  &
         'Sea Ice Area Fraction', '1.0', standard_name='sea_ice_area_fraction' )

    if ( id_sic > 0 ) &
         call fms_diag_field_add_attribute(id_sic, 'comment', 'averaged over the ocean portion of grid box')

    !----- initialize global integrals for netCDF output -----
    id_evspsbl_g = register_global_diag_field(&
         'evspsbl', Time, 'Evaporation', 'mm d-1', standard_name='water_evaporation_flux')

    id_ts_g = register_global_diag_field(&
         'ts', Time, 'Surface Temperature', 'K', standard_name='surface_temperature')

    id_tas_g = register_global_diag_field(&
         'tas', Time, 'Near-Surface Air Temperature', 'K', standard_name='air_temperature')
    ...
    id_tasl_g = register_global_diag_field(&
         'tasl', Time, 'Near-Surface Air Temperature (Land Only)', 'K' , standard_name='air_temperature')
    ...
    id_rls_g = register_global_diag_field('rls', Time, &
         'Net Longwave Surface Radiation', 'W m-2', standard_name='surface_net_longwave_flux')

#endif

  end subroutine diag_field_init

  !> Subroutine divide_by_area divides data on a grid by the grid cell area for cells
  !! with non-zero area
  subroutine divide_by_area(data, area)
    ...
    !{
    if(size(data, dim=1) /= size(area, dim=1) .or. size(data, dim=2) /= size(area, dim=2)) return
    !}
  ...
  !! If the ice_mask or sic variable have been registered with diag_manager,
  ...
  SUBROUTINE SEND_ICE_MASK_SIC(TIME)
    ...
    if(id_ice_mask > 0 .or. id_sic > 0) then
       ...
       !> MAP ICE_FRAC FROM THE OCN GRID TO THE EXCHANGE GRID
       !{
       call fms_xgrid_put_to_xgrid(ice_frac, 'OCN', ex_ice_frac, xmap_sfc)
       !}

       !> MAP ICE_FRAC FROM THE EXCHANGE GRID TO THE ATM GRID
       !{
  ...
  subroutine atm_stock_integrate(Atm, res)
    ...
    implicit none
    type(atmos_data_type), intent(in) :: Atm !< derived type holding the atmosphere boundary data
    real, intent(out) :: res !< integrated value

    integer :: ier ! error returned from fms_xgrid_stock_integrate_2d
