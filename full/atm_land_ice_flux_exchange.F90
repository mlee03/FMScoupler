...
module atm_land_ice_flux_exchange_mod
  ...
  !< is a Github tag that is set automatically during compile time
  ...
  !< is a diag_manager register field id for 'near-surface air temperature a
  ...
  !< is a diag_manager register field id for 'concentration of o2 to be passed to ocean'
  ...
  !< is a flag where if true, land_mask, sftlf, height2m, and height10m are saved once per file
  !! at first call to sf_boundary_layer
  ...
  !< is the number of blocks in OpenMP parallelization, defaults to 1
  ...
  !! Used in forecast mode (do_forecast=.true.) when #ifdef AM3_physics is defined
  ...
  !< is the fixed (overridden) direct visible-band surface albedo on the exchange grid [dimensionless];
  !! used to apply data_override corrections to the albedo before flux calculations

  real, allocatable, dimension(:) :: ex_albedo_nir_dir_fix
  !< is the fixed (overridden) direct near-infrared surface albedo on the exchange grid [dimensionless]

  real, allocatable, dimension(:) :: ex_albedo_vis_dif_fix
  !< is the fixed (overridden) diffuse visible-band surface albedo on the exchange grid [dimensionless]

  real, allocatable, dimension(:) :: ex_albedo_nir_dif_fix
  !< is the fixed (overridden) diffuse near-infrared surface albedo on the exchange grid [dimensionless]
  ...
  !< is the implicit coupling coefficient e_t^n for sensible heat on the exchange grid;
  !! used in the semi-implicit surface flux scheme to couple atmospheric temperature to surface temperature

  real, allocatable, dimension(:) :: ex_f_t_delt_n
  !< is the implicit coupling coefficient f_t^{delta n} for sensible heat on the exchange grid;
  !! represents the accumulated forcing term in the semi-implicit time integration of surface heat flux
  ...
  type :: tracer_ind_type
     integer :: atm !< Tracer index in the atmosphere model tracer table (NO_TRACER if not present)
     integer :: ice !< Tracer index in the sea-ice model tracer table (NO_TRACER if not present)
     integer :: lnd !< Tracer index in the land model tracer table (NO_TRACER if not present)
  ...
  type :: tracer_exch_ind_type
     integer :: exch = 0 !< Index of this atmosphere tracer in the exchange tracer table tr_table (0 if not exchanged)
     integer :: ice  = 0 !< Corresponding tracer index in the sea-ice model (0 if not present)
     integer :: lnd  = 0 !< Corresponding tracer index in the land model (0 if not present)
...
contains
  ...
  subroutine atm_land_ice_flux_exchange_init(Time, Atm, Land, Ice, atmos_ice_boundary, land_ice_atmos_boundary, &
    ...
    !< is used to set scale_precip_2d in the module where if true, rescale Atm%lprec
    logical, intent(in) :: do_area_weighted_flux_in
    !< is used to set do_area_weighted_flux in the module where if true, divide flux by area
  ...
  !!
  !!   t_surf_atm, albedo_atm, rough_mom_atm, land_frac_atm, dtaudu_atm, dtaudv_atm, flux_u_atm, flux_v_atm
  !!   u_star_atm, b_star_atm
  !!
  !! Note, u_star and b_star are defined so that u_star**2 is the magnitude of surface stress
  ...
  subroutine sfc_boundary_layer ( dt, Time, Atm, Land, Ice, Land_Ice_Atmos_Boundary )
    ...
    real, dimension(n_xgrid_sfc) :: &
         ...
         ex_cd_q, & ! moisture exchange coefficien
         ex_ref, &! specific humidity at z_ref_hea
    ...
    real :: rho  ! Air density at the surface (kg/m3); used to convert deposition velocity to tracer concentration flux
    real, dimension(n_xgrid_sfc,n_exch_tr) :: &
         ex_tr_atm, & ! Tracer mixing ratio at the lowest atmospheric level on the exchange grid [kg/kg]
         ex_tr_ref    ! Tracer mixing ratio at the reference height z_ref_heat on the exchange grid [kg/kg]

    real, dimension(n_xgrid_sfc) :: &
         ex_co2_atm_dvmr ! CO2 dry volume mixing ratio at lowest atmospheric level on exchange grid [mol/mol]
    real, dimension(size(Land_Ice_Atmos_Boundary%t,1),size(Land_Ice_Atmos_Boundary%t,2)) :: &
         diag_atm ! Scratch array on the atmosphere grid for computing and sending diagnostic quantities
    ...
    real, dimension(size(Ice%t_surf,1),size(Ice%t_surf,2),size(Ice%t_surf,3)) :: &
         sea ! Scratch array set to 1.0 in the open-water ice category and 0.0 elsewhere; used to construct ex_seawater

    real, dimension(size(Ice%albedo,1),size(Ice%albedo,2),size(Ice%albedo,3)) :: &
         tmp_open_sea ! Scratch array set to 1.0 in the first ice category (open water); used to
                      !! construct ex_frac_open_sea

    real :: zrefm ! Momentum reference height for Monin-Obukhov flux calculations [m]; passed to surface_flux
    real :: zrefh ! Heat/moisture reference height for Monin-Obukhov flux calculations [m]; passed to surface_flux
    logical :: used ! Return flag from fms_data_override; .true. if the field was overridden from the data table
    character(32) :: tr_name, tr_units ! Tracer name and physical units strings retrieved from tracer manager
    integer :: tr, n, m ! Tracer index, boundary-condition index, and field index loop variables
    integer :: is, ie ! x-direction loop bounds on the exchange grid (OpenMP block start/end)
    integer :: isc, iec, jsc, jec ! Compute-domain x/y index bounds
    integer :: i, l, j, n_gex ! General do-loop indices (exchange-grid cell, block, latitude, generic-exchange field)

    real, dimension(n_xgrid_sfc,n_gex_lnd2atm) ::  ex_gex_lnd2atm
    ! Generic (non-tracer) fields exchanged from land to atmosphere on exchange grid
    ...
    !! @endparblock
    ! [3] initialize some values on exchange grid: this is actually a safeguard against using undefined values
    ...
    !> @parblock
    !! OVERRIDE ATM FIELDS IF FIELD EXISTS IN THE DATA_OVERRIDE DATA_TABLE.
    !! @endparblock
    ...
    !> @parblock
    !! OVERRIDE CO2 VALUES IF THE FIELD EXISTS IN DATA_OVERRIDE DATA_TABLE
    !! AND SEND DATA TO THE DIAG_MANAGER BUFFER.
    !! @endparblock
    ...
    !> @parblock
    !! OVERRIDE ICE AND LAND FIELD IF THE FIELD EXISTS IN DATA_OVERRIDE DATA_TABLE.
    !! @endparblock
    ...
    !> @parblock
    !! ON THE EXCHANGE GRID, INITIALIZE CANOPY TEMPERATURE EQUAL TO SURFACE TEMPERATURE.
    !! @endparblock
    ...
    !> @parblock
    !! OVERRIDE LAND AND ICE TRACER FLUXES IF FIELD EXISTS IN DATA_OVERRIDE DATA_TABLE.
    !! @endparblock
    ...
    !! NOTE, TO COMPUTE FLUXES, T_SURF**4 (T_SURF TO THE FOURTH POWER) IS SENT TO THE EXCHANGE GRID
    !! AND NOT T_SURF DUE TO NONLINEARITY IN THE STEFAN-BOLTZMANN LAW WHERE
    !! LOGWAVE_FLUX = STEFAN_BOLTZMANN_CONSTANT * T**4.
    ...
    !> @parblock
    !! MAP FIELDS FROM THE EXCHANGE GRID TO THE ATM GRID.
    !! @endparblock
...
! for shield+mom6 coupling; used by shield pbl schemes (am5 with tke-edmf should do the same)
    ...
    !> @parblock
    !! DATA OVERRIDE ATMOSPHERIC QUANTITIES IF FOUND IN DATA_TABLE.
    !! @endparblock
    ...
    !> @parblock
    !! MAP ATMOSPHERIC DATA FROM THE EXCHANGE TO ATM GRID AND SEND DATA TO DIAG_MANAGER BUFFER.
    !! @endparblock
    !------- Atm fields -----------
    do n = 1, Atm%fields%num_bcs
       do m = 1, Atm%fields%bc(n)%num_fields
  ...
  subroutine flux_down_from_atmos (Time, Atm, Land, Ice, Atmos_boundary, Land_boundary, Ice_boundary )
    ...
    !< is a derived type holding properties and fluxes passed from atmosphere to land
    ...
    !> @parblock
    !! OVERRIDE ATM SHORTWAVE/LONGWAVE, DIRECT/DOWNWARD DIFFUSIVE FLUXES
    !! NOTE, DATA_OVERRIDE WILL ONLY OVERWRITE IF THE FIELD IS SPECIFIED IN THE DATA_TABLE.
    !! @endparblock
    ! fms_data_override calls moved here from coupler_main
    ...
    !> @parblock
    !! ON THE EXCHANGE GRID, TAKE INTO ACCOUNT FOR ALBEDO VARIATION IN SHORTWAVE RADIATION FLUX OF VISIBLE LIGHT
    !! @endparblock
    ...
    !> @parblock
    !! COMPUTE STOCK EXCHANGES BETWEEN COMPONENTS
    !! @endparblock
    ! Atm -> Lnd (precip)
#ifndef _USE_LEGACY_LAND_
  ...
  !> @parblock
  !! Subroutine generate_sfc_xgrid updates fractional areas on the surface exchange grid and
  !! recompute the number of active exchange-grid cells.  The fractional area measures the portion
  !! of each exchange-grid cell that corresponds to land or ice.  fms_xgrid_set_frac_area is called
  !! for both the OCN (ice) and LND (land) grids to reflect the current sea-ice concentration and
  !! land-tile coverage.
   !! @endparblock
  ...
  subroutine flux_up_to_atmos ( Time, Land, Ice, Land_Ice_Atmos_Boundary, Land_boundary, Ice_boundary )
    ...
    !< is a derived type holding ice boundary data
    type(land_ice_atmos_boundary_type), intent(inout) :: Land_Ice_Atmos_Boundary
    !< is a derived type holding properties and fluxes passed from exchange grid to the atmosphere, land and ice
    type(atmos_land_boundary_type), intent(inout) :: Land_boundary
    !< is a derived type holding properties and fluxes passed from atmosphere to land
    type(atmos_ice_boundary_type),  intent(inout) :: Ice_boundary
    !< is a derived type holding properties and fluxes passed from atmosphere to ice
  ...
  !> @parblock
  !! Subroutine flux_ex_arrays_dealloc deallocates the model-level exchange grid related
  !! arrays that were allocated in sfc_boundary_layer
   !! @endparblock
  subroutine flux_ex_arrays_dealloc()

    integer :: m,n  !< Boundary-condition and field loop indices for deallocating gas-flux coupler-type arrays
  ...
  subroutine flux_atmos_to_ocean(Time, Atm, Ice_boundary, Ice)
    ...
    !! ARE SPECIFIED IN THE DATA_TABLE.  THEN SEND_DATA TO THE DIAG_MANAGER BUFFER.
    !! @endparblock
    do n = 1, Ice_boundary%fluxes%num_bcs
  ...
  !! where .true. is equal to 1.0 and .false. is equal to 0.0.  The real array is then mapped
  ...
  !! must be specified in diag_table.yaml in order for the data to be outputted
  ...
  subroutine diag_field_init ( Time, atmos_axes, land_axes, land_pe )
    ...
    integer :: iref           ! Rounded integer version of z_ref_heat or z_ref_mom for label formatting
    character(len=6) :: label_zm ! Formatted string for z_ref_mom used in diagnostic field long names (e.g. "10 m")
    character(len=6) :: label_zh ! Formatted string for z_ref_heat used in diagnostic field long names (e.g. "2 m")
    real, dimension(2) :: trange = (/  100., 400. /), & ! Valid range for temperature diagnostics [K]
         vrange = (/ -400., 400. /), &                  ! Valid range for wind/flux diagnostics
         frange = (/ -0.01, 1.01 /)                     ! Valid range for fractional (0-1) diagnostics
    character(len=32)  :: name, units ! Tracer name and units strings for diagnostic registration
    character(len=128) :: longname    ! Long name of the tracer for diagnostic registration
    integer            :: tr          ! Tracer loop index
    integer            :: area_id     ! Diagnostic field ID for grid-cell area (used internally)
...
!f1p
  ...
  subroutine atm_stock_integrate(Atm, res)
    ...
    integer :: ier  !< Error code returned by fms_xgrid_stock_integrate_2d (0 = success)
